...
public class Main {
    ...
    private static void moving(int[][] board, Position[] horse, int[] move, int score, int depth) {
        ...
        for(int i=0; i<4; i++) {
            ...
            if(cur.y == board[cur.x].length-1){
                continue;
            }
            int x = cur.x;
            int y = cur.y + move[depth] >= board[cur.x].length ? board[cur.x].length -1 : cur.y + move[depth];
            if(board[x][y] % 10 == 0 && board[x][y] != 40 && x == 0){
                x = board[x][y]/10;
                y = 0;
            }
            if(check(board, horse, i, x, y)){
    ...
    private static boolean check(int[][] board, Position[] horse, int target, int x, int y) {
        ...
        for(int i=0; i<4; i++) {
            ...
            if (horse[i].x == x && horse[i].y == y){
                return false;
            }
            if((board[x][y]== 25 && board[horse[i].x][horse[i].y] == 25) || (board[x][y]== 40 && board[horse[i].x][horse[i].y] == 40)){
                return false;
            }
        }
        return true;
